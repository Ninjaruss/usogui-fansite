...
export class BackblazeB2Service {
  ...
  private authData: { authorizationToken: string; apiUrl: string; downloadUrl: string } | null = null;
  ...
  private get applicationKeyId(): string {
    ...
    if (!keyId) {
      throw new InternalServerErrorException('B2_APPLICATION_KEY_ID not configured');
    }
    return keyId;
  }
  ...
  private get applicationKey(): string {
    ...
    if (!key) {
      throw new InternalServerErrorException('B2_APPLICATION_KEY not configured');
    }
    return key;
  }
  ...
  private async authorize(): Promise<{ authorizationToken: string; apiUrl: string; downloadUrl: string }> {
    ...
    try {
      ...
      if (!authData.apiUrl) {
        this.logger.error('B2 authorization response missing apiUrl field', data);
      ...
      throw new InternalServerErrorException('Failed to authorize with Backblaze B2');
    }
  }

  private async ensureAuth(): Promise<{ authorizationToken: string; apiUrl: string; downloadUrl: string }> {
  ...
  private async getUploadUrl(): Promise<B2UploadUrlResponse> {
    ...
    if (!auth.apiUrl) {
      this.logger.error('Cannot get upload URL: apiUrl is undefined in auth data');
      throw new InternalServerErrorException('B2 authorization data is invalid - missing apiUrl');
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(`Failed to get upload URL: ${response.status} ${response.statusText}`, errorText);
  ...
  async uploadFile(
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        this.logger.error(`B2 upload failed: ${response.status} ${response.statusText}`, errorText);
  ...
  async deleteFile(fileName: string): Promise<void> {
    ...
    try {
      ...
      const listData = await listResponse.json() as { files: Array<{ fileId: string; fileName: string }> };
  ...
  generateSignedUrl(fileName: string, _expiresInSeconds: number = 3600): string {
