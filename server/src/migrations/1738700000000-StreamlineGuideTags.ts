...
export class StreamlineGuideTags1738700000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      USING "guide_tags" b
      WHERE a.ctid < b.ctid AND a."guideId" = b."guideId" AND a."tagId" = b."tagId"
    `);

    // Remap tag_events_event if it exists
    const tagEventsExists = await queryRunner.query(`
    ...
    if (tagEventsExists[0]?.exists) {
      ...
      await queryRunner.query(`
        ...
        WHERE a.ctid < b.ctid AND a."eventId" = b."eventId" AND a."tagId" = b."tagId"
      `);
    }

    // Delete old tags
    await queryRunner.query(`
      DELETE FROM "tag" WHERE name NOT IN ('Gamble Breakdown', 'Character Study', 'Plot Analysis')
    `);
  }
