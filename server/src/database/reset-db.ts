...
async function resetDatabase() {
  console.log(chalk.yellow('⚠️ WARNING: This will completely reset your database!'));
  console.log(chalk.yellow('⚠️ All data will be lost!'));
  
  // Check if we're in production
  if (process.env.NODE_ENV === 'production') {
    console.log(chalk.red('❌ Refusing to reset database in production environment'));
    console.log(chalk.red('❌ This operation is only allowed in development or test environments'));
    process.exit(1);
  }
  ...
  if (process.env.ENABLE_SCHEMA_SYNC !== 'true') {
    ...
    console.log(chalk.red('❌ Set ENABLE_SCHEMA_SYNC=true to allow database reset'));
    process.exit(1);
  }
